...
- **Always use Brave browser cookies**: Add `--cookies-from-browser brave` to ALL yt-dlp commands. YouTube requires authentication and Brave is the installed browser.
- **Always download videos as MP4**: When downloading video files (not just subtitles), always use `--merge-output-format mp4` to ensure MP4 output.
...
```bash
yt-dlp --cookies-from-browser brave --list-subs "YOUTUBE_URL"
```
...
```bash
yt-dlp --cookies-from-browser brave --write-sub --skip-download --output "OUTPUT_NAME" "YOUTUBE_URL"
```
...
```bash
yt-dlp --cookies-from-browser brave --write-auto-sub --skip-download --output "OUTPUT_NAME" "YOUTUBE_URL"
```
...
yt-dlp --cookies-from-browser brave --print "%(filesize,filesize_approx)s" -f "bestaudio" "YOUTUBE_URL"

# Or get duration to estimate
yt-dlp --cookies-from-browser brave --print "%(duration)s %(title)s" "YOUTUBE_URL"
```
...
```bash
yt-dlp --cookies-from-browser brave -x --audio-format mp3 --output "audio_%(id)s.%(ext)s" "YOUTUBE_URL"
```
...
```bash
yt-dlp --cookies-from-browser brave --print "%(title)s" "YOUTUBE_URL"
```

Use this to create meaningful filenames based on the video title. Clean the title for filesystem compatibility:
- Replace `/` with `-`
- Replace special characters that might cause issues
- Consider using sanitized version: `$(yt-dlp --cookies-from-browser brave --print "%(title)s" "URL" | tr '/' '-' | tr ':' '-')`
...
```bash
# Get video title
VIDEO_TITLE=$(yt-dlp --cookies-from-browser brave --print "%(title)s" "YOUTUBE_URL" | tr '/' '_' | tr ':' '-' | tr '?' '' | tr '"' '')
...
VIDEO_TITLE=$(yt-dlp --cookies-from-browser brave --print "%(title)s" "$VIDEO_URL" | tr '/' '_' | tr ':' '-' | tr '?' '' | tr '"' '')
...
if yt-dlp --cookies-from-browser brave --write-sub --skip-download --output "$OUTPUT_NAME" "$VIDEO_URL" 2>/dev/null; then
    ...
    if yt-dlp --cookies-from-browser brave --write-auto-sub --skip-download --output "$OUTPUT_NAME" "$VIDEO_URL" 2>/dev/null; then
        ...
        # Get file size
        FILE_SIZE=$(yt-dlp --cookies-from-browser brave --print "%(filesize_approx)s" -f "bestaudio" "$VIDEO_URL")
        DURATION=$(yt-dlp --cookies-from-browser brave --print "%(duration)s" "$VIDEO_URL")
        TITLE=$(yt-dlp --cookies-from-browser brave --print "%(title)s" "$VIDEO_URL")
        ...
        if [[ "$RESPONSE" =~ ^[Yy]$ ]]; then
            ...
            yt-dlp --cookies-from-browser brave -x --audio-format mp3 --output "audio_%(id)s.%(ext)s" "$VIDEO_URL"
